...
const newsUrl = 'https://cors-anywhere.herokuapp.com/https://newsapi.org/v2/everything?apiKey=45bbd722e9ed4d88887103aa39f34a4f&q=covid-19'
...
const getRestaurantData = async (borough) => {
  ...
  try {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      // Putting restaurant name into title case, got help from https://www.freecodecamp.org/news/three-ways-to-title-case-a-sentence-in-javascript-676a9175eb27/
      let restNameTitleCase = ranRestObj.restaurant_name.toLowerCase().split(' ')
      for (let j = 0; j < restNameTitleCase.length; j++) {
        restNameTitleCase[j] = restNameTitleCase[j].charAt(0).toUpperCase() + restNameTitleCase[j].slice(1)
      }
      restNameTitleCase = restNameTitleCase.join(' ')

      // Replacing characters
      let newRestaurantName = restNameTitleCase.replace("ï¿½", "'");

      // Putting address into title case, got help from https://www.freecodecamp.org/news/three-ways-to-title-case-a-sentence-in-javascript-676a9175eb27/
      let addressTitleCase = ranRestObj.business_address.toLowerCase().split(' ')
      for (let k = 0; k < addressTitleCase.length; k++) {
        addressTitleCase[k] = addressTitleCase[k].charAt(0).toUpperCase() + addressTitleCase[k].slice(1)
      }
      addressTitleCase = addressTitleCase.join(' ')

      // Appending restaurant name and class
      nameOfRestaurant.textContent = newRestaurantName
      ...
      businessAddress.textContent = addressTitleCase
